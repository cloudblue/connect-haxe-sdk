	...
	**/
	...
	public function _run<T>(list:Collection<T>):Void {
		...
		final filteredList = (filterFunc != null) ? Collection._fromArray(list.toArray().filter(#if cslib(m) -> filterFunc.Invoke(cast(m,
			IdModel)) #elseif java(m) -> filterFunc.apply(cast(m, IdModel)) #else(m) -> filterFunc(cast(m, IdModel)) #end)) : list;
	...
	private function processAbortAndCloseLogSection(index:Int, requestStr:String, dataStr:String):{nextIndex:Int, lastRequestStr:String, lastDataStr:String} {
		if (this.abortRequested) {
			if (this.abortMessage == null) {
				...
				final saveResult = StepStorage.save(this.model, new StepData(index, this.data, ConnectStorage), param,
	...
	**/
	...
	private function logStepData(level:Int, request:String, data:String, lastRequest:String, lastData:String) {
		for (output in Env.getLogger().getOutputs()) {
			final list = new Collection<String>().push(getFormattedRequest(request, lastRequest, output.formatter))
